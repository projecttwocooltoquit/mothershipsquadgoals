...
//     type: 'list',
//     name: '',
//     message: 'choose player 1 character',
//     options: heroChoices,
//   },
// ]);
​
...
for(i=0;i<heroes.length;i++){
  if (typeof heroes[i].powerstats !== 'undefined' && heroes[i].powerstats.speed !== 'null'){
      validCharacters.push(heroes[i]);
  }
};
​
let pOneRandom = validCharacters[Math.floor(Math.random() * validCharacters.length)]
let pTwoRandom = validCharacters[Math.floor(Math.random() * validCharacters.length)]
...
const playerOneRandom = new Character(pOneRandom.name, pOneRandom.powerstats.strength, pOneRandom.powerstats.durability);
const playerTwoRandom = new Character(pTwoRandom.name, pTwoRandom.powerstats.strength, pTwoRandom.powerstats.durability);
​
